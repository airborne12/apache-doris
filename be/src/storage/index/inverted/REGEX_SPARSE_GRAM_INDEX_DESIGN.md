...
- 在 httplogs（低熵 URL）上 CDC 体积 20.1% 但只有 12/15 可索引、中位 1.4×（该负载本身宽泛：7/15 的查询命中 ≥3%）；稠密 3-gram 57% 得 4.2×。低熵短串列应选 `dense_gram` 并配 τ 裁剪（df>50% 的 21 个 gram 占 posting 的 50%）。
...
| 低熵短串（URL、路径、枚举型） | `dense_gram`, n=3, τ=0.25 | 30–45% | 仅选择性查询受益（4–200×），宽泛查询 1–2× |
